	...
	action create_electric_taxis_batch(int how_many) {
	    ...
	    if (to_create > 0) {
	        create electricCars number: to_create returns: created_taxis {
	            initialCrossroad <- one_of(crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation));
	            self.location <- initialCrossroad.location;
	            targetCrossroads <- one_of(crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation));
...
species crossroads skills: [intersection_skill] {
    ...
    action initialize {
        ...
        if (isStreet or isBusStop or isNotFinishPoint or isTurningCircle) {
...
species train skills: [driving] {
        ...
        if (contStop < timeToStopInAStop) {
            ...
            if (current_target != final_target) {
                ...
                loop ps over: passengersGettingOff {
                    building arrival_station <- ps.the_target;
                    if (arrival_station = nil) {
                        arrival_station <- one_of(building where (
                            (each.buildingType = "train_station" or each.railwayType = "station" or each.railwayType = "platform")
                            and nextCrossroad.nameTrainStation != nil
                            and each.buildingName = nextCrossroad.nameTrainStation
                        ));
                    }
                    if (arrival_station = nil) {
                        arrival_station <- (building where (
                            each.buildingType = "train_station" or each.railwayType = "station" or each.railwayType = "platform"
                        ) closest_to point(current_target));
                    }
                    if (arrival_station != nil) {
                        ps.location <- point(arrival_station);
                    } else {
                        ps.location <- point(current_target);
                    }
                    ps.stationAgent <- current_target;
                ...
                loop ps over: passengers {
                    building arrival_building <- (ps.the_final_target != nil) ? ps.the_final_target : ps.the_target;
                    if (ps.the_final_target != nil) {
                        ps.the_target <- ps.the_final_target;
                    }
                    ask ps { do settle_trip_state(arrival_building, "COMPLETED"); }
                }
                passengers <- [];
            }
            // Board waiting passengers
            loop boarder over: nextCrossroad.waitingPassengers {
                if (boarder != nil) {
                    boarder.location <- self.location;
                    boarder.stationAgent <- nextCrossroad;
                    boarder.waitingForTrain <- true;
                    boarder.startJourney <- false;
                    boarder.isMoving <- false;
                    boarder.stopped <- true;
                }
            }
    ...
    reflex handleBreakdown when: breakdown {
        ...
        if (contStop >= 3.0 #mn) {
            loop p over: passengers {
                building intended_destination <- (p.the_final_target != nil) ? p.the_final_target : p.the_target;
                ask p { do settle_trip_state(intended_destination, "ABORTED_TRAIN_BREAKDOWN"); }
            }
...
species vehicles skills: [driving] {
    ...
    path compute_road_path_with_fallback(crossroads source_node, crossroads destination_node) {
        if (source_node = nil or destination_node = nil) { return nil; }
        point previous_location <- location;
        if (source_node.location != nil) {
            location <- source_node.location;
        }
        path candidate_path <- compute_path(graph: roadsNetwork, target: destination_node);
        if (candidate_path = nil and previous_location != nil) {
            location <- previous_location;
        }
        return candidate_path;
    }

    list<crossroads> ordered_drive_candidates(point anchor_location) {
        if (anchor_location = nil) { return []; }
        return crossroads where (
            each != nil
            and each.location != nil
            and !each.isStreet
            and !(each.crossroadsNoInitialLocation)
        ) sort_by (distance_to(each.location, anchor_location));
    }
    ...
    reflex handleRemoval when: breakdown {
        ...
        if (breakdown_timer >= breakdown_duration) {
            ...
            loop p over: passengers {
                building intended_destination <- (p.the_final_target != nil) ? p.the_final_target : p.the_target;
                ask p { do settle_trip_state(intended_destination, "ABORTED_" + (myself.breakdown ? "BREAKDOWN" : "STUCK")); }
...
species normalCars parent: vehicles {
    ...
    reflex timeToGoNormalCars when: final_target = nil and carStopInAZebraCrossing = false and carStopInAStop = false {
        ...
        if (location != nil) {
            crossroads closest_source <- (crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation) closest_to self.location);
        ...
        if (targetCrossroads = nil) {
            targetCrossroads <- (crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation) closest_to self.location);
            failed_route_destination <- targetCrossroads;
        }
        routePlanAttempts <- 0;
        routePlanOutcomeLogged <- false;
        ...
        if (route_source != nil and targetCrossroads != nil and route_source = targetCrossroads) {
            ...
            loop p over: passengers {
                if (p != nil) {
                    building arrival_building <- (p.the_target != nil) ? p.the_target : p.living_place;
                    ask p { do settle_trip_state(arrival_building, "COMPLETED"); }
                }
            }
            do die;
        }
        ...
        bool planner_exhausted_candidates <- false;
        point source_anchor <- (route_source != nil and route_source.location != nil) ? route_source.location : self.location;
        point target_anchor <- (targetCrossroads != nil and targetCrossroads.location != nil) ? targetCrossroads.location : source_anchor;
        list<crossroads> ordered_origin_candidates <- ordered_drive_candidates(source_anchor);
        list<crossroads> ordered_destination_candidates <- ordered_drive_candidates(target_anchor);
        crossroads selected_origin <- nil;
        crossroads selected_destination <- nil;
        int origin_cursor <- 0;
        int destination_cursor <- 0;
        if (route_source != nil and targetCrossroads != nil) {
            loop while: current_path = nil
                and routePlanAttempts < routePlanFailureThreshold
                and origin_cursor < length(ordered_origin_candidates)
                and destination_cursor < length(ordered_destination_candidates) {
                selected_origin <- ordered_origin_candidates[origin_cursor];
                selected_destination <- ordered_destination_candidates[destination_cursor];
                failed_route_origin <- selected_origin;
                failed_route_destination <- selected_destination;
                planned_car_origin <- selected_origin;
                planned_car_destination <- selected_destination;
                routePlanAttempts <- routePlanAttempts + 1;
                current_path <- compute_road_path_with_fallback(selected_origin, selected_destination);
                if (current_path != nil) {
                    if (!empty(current_path.edges)) {
                        current_road <- roads(current_path.edges[0]);
                    }
                    location <- selected_origin.location;
                    initialCrossroad <- selected_origin;
                    targetCrossroads <- selected_destination;
                } else {
                    current_road <- nil;
                    origin_cursor <- origin_cursor + 1;
                    destination_cursor <- destination_cursor + 1;
                }
            }
            planner_exhausted_candidates <- (
                current_path = nil
                and (
                    origin_cursor >= length(ordered_origin_candidates)
                    or destination_cursor >= length(ordered_destination_candidates)
                )
            );
        } else {
            planner_exhausted_candidates <- true;
        }
        ...
        if (!planner_state_ready and (routePlanAttempts >= routePlanFailureThreshold or planner_exhausted_candidates)) {
            if (!routePlanOutcomeLogged) {
            ...
            loop p over: passengers {
                if (p != nil) {
                    building intended_destination <- (p.the_final_target != nil) ? p.the_final_target : p.the_target;
                    ask p { do settle_trip_state(intended_destination, "ABORTED_NO_ROUTE_CAR"); }
                }
            }
            do die;
        }
    ...
    reflex moveNormalCars when: current_path != nil and final_target != nil and carStopInAStop = false and carStopInAZebraCrossing = false and carStopInAYield = false and !breakdown {
        ...
        if (current_path != nil and (empty(current_path.edges) or current_road = nil)) {
            if (final_target.location != nil) {
                location <- final_target.location;
            }
            current_path <- nil;
            current_road <- nil;
            final_target <- nil;
        ...
        if (current_path != nil and current_road != nil and final_target != nil) {
    ...
    reflex removeStuckNormalCar when: !breakdown
        ...
        loop p over: passengers {
            if (p != nil) {
                building intended_destination <- (p.the_final_target != nil) ? p.the_final_target : p.the_target;
                ask p { do settle_trip_state(intended_destination, "ABORTED_STUCK_CAR"); }
            }
        }
        passengers <- [];
...
species electricCars parent: vehicles {
    ...
    action set_taxi_phase_target {
        if (headingToPickUpPassenger and !empty(passengersToPickUp)) {
            ...
            if (p_pick != nil) {
                targetCrossroads <- (crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation)) closest_to p_pick.location;
            }
            ...
            if (p_drop != nil and p_drop.the_target != nil) {
                targetCrossroads <- (crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation)) closest_to p_drop.the_target.location;
            }
    ...
    action abort_active_taxi_trip_no_route(string reason_label) {
        ...
        loop p over: passengers {
            if (p != nil) {
                building intended_destination <- (p.the_final_target != nil) ? p.the_final_target : p.the_target;
                ask p { do settle_trip_state(intended_destination, "ABORTED_NO_ROUTE_TAXI"); }
            }
        }
        passengers <- [];
        ...
        loop p over: passengersToPickUp {
            if (p != nil) {
                building intended_destination <- (p.the_final_target != nil) ? p.the_final_target : p.the_target;
                ask p { do settle_trip_state(intended_destination, "ABORTED_NO_ROUTE_TAXI_PICKUP"); }
            }
        }
        passengersToPickUp <- [];
    ...
    when: lowBattery = false
       ...
       and carStopInAZebraCrossing = false {
        ...
        if (soc < 0.30) {
            ...
            crossroads route_source <- (crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation)) closest_to self.location;
            point source_anchor <- (route_source != nil and route_source.location != nil) ? route_source.location : self.location;
            point target_anchor <- (targetCrossroads != nil and targetCrossroads.location != nil) ? targetCrossroads.location : source_anchor;
            list<crossroads> ordered_origin_candidates <- ordered_drive_candidates(source_anchor);
            list<crossroads> ordered_destination_candidates <- ordered_drive_candidates(target_anchor);
            ...
            if (route_source != nil and targetCrossroads != nil) {
                loop while: current_path = nil
                    and route_attempts < routePlanFailureThreshold
                    and origin_cursor < length(ordered_origin_candidates)
                    and destination_cursor < length(ordered_destination_candidates) {
                    selected_origin <- ordered_origin_candidates[origin_cursor];
                    selected_destination <- ordered_destination_candidates[destination_cursor];
                    ...
                    if (current_path != nil) {
                        ...
                        location <- selected_origin.location;
                        targetCrossroads <- selected_destination;
                        final_target <- selected_destination;
                    } else {
                        current_road <- nil;
                        origin_cursor <- origin_cursor + 1;
                        destination_cursor <- destination_cursor + 1;
            ...
            if (taxi_trip_active and current_path = nil and (route_attempts >= routePlanFailureThreshold or planner_exhausted_candidates)) {
                do abort_active_taxi_trip_no_route("no_route_blacklist");
            }
        }
    }
    ...
    when: lowBattery = true
       ...
       bool teleported_to_charger <- false;
       int charge_candidate_cursor <- 0;
       crossroads route_source <- (crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation)) closest_to self.location;
       current_path <- nil;
       current_road <- nil;
       if (route_source != nil) { location <- route_source.location; }
       loop while: !found_route and candidates_tested < routePlanFailureThreshold and charge_candidate_cursor < length(chargeCandidates) {
           crossroads candidate <- chargeCandidates[charge_candidate_cursor];
           candidates_tested <- candidates_tested + 1;
           current_path <- compute_road_path_with_fallback(route_source, candidate);
           if (current_path != nil) {
               if (!empty(current_path.edges)) {
                   current_road <- roads(current_path.edges[0]);
               }
               selected_charge_candidate <- candidate;
               found_route <- true;
           } else {
               current_road <- nil;
               charge_candidate_cursor <- charge_candidate_cursor + 1;
           }
       }
       ...
       if (found_route) {
           ...
           if (!empty(chargeCandidates)) {
               ...
               closestChargingPoint <- selected_charge_candidate;
               self.location <- selected_charge_candidate.location;
               final_target <- nil;
               carStopInAElectricRecharge <- true;
               teleported_to_charger <- true;
               numTimesCurrentPathNull <- 0;
           } else {
               numTimesCurrentPathNull <- numTimesCurrentPathNull + 1;
               final_target <- nil;
       ...
       if (simLogger != nil) {
           string plan_result <- teleported_to_charger ? "teleport_fallback"
               : (found_route ? "first_try_success" : "failed_after_retries");
       ...
       if (teleported_to_charger) {
           write "" + self + " teleported directly to charging point after failing all charging routes.";
       } else if (found_route) {
    ...
    when: current_path = nil
       ...
       if ((contStop >= timeToStopInAElectricRecharge) or (soc >= 0.80)) {
           ...
           self.location <- ((crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation)) closest_to self.location).location;
           final_target <- nil;
           isAvailable <- true;
           isWandering <- true;
           headingToChargingPoint <- false;
            ...
            // Arrived or no path to advance
               ...
               if (headingToPickUpPassenger) {
                   loop p over: passengersToPickUp {
                       ...
                       if (p.the_target != nil) {
                           targetCrossroads <- (crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation)) closest_to p.the_target.location;
                           currentTrip.pickupTime <- current_date;
                       }
                   }
                   headingToPickUpPassenger <- false;
    ...
    reflex removeStuckElectricTaxi when: !breakdown
       ...
       and counterStucked >= stuck_removal_duration {
        ...
        loop p over: passengers {
            if (p != nil) {
                building intended_destination <- (p.the_final_target != nil) ? p.the_final_target : p.the_target;
                ask p { do settle_trip_state(intended_destination, "ABORTED_STUCK_TAXI"); }
            }
        }
        passengers <- [];
        ...
        loop p over: passengersToPickUp {
            if (p != nil) {
                building intended_destination <- (p.the_final_target != nil) ? p.the_final_target : p.the_target;
                ask p { do settle_trip_state(intended_destination, "ABORTED_STUCK_TAXI_PICKUP"); }
            }
        }
        passengersToPickUp <- [];
...
species Person skills: [moving] {
    ...
    bool mandatory_home_return_pending <- false;
    ...
    action end_trip_log(string status) {
        ...
        if (current_purpose = "resting") {
            mandatory_home_return_pending <- false;
            inActivity <- false;
            activity_partners <- [];
        }
        current_trip_id <- nil;
        current_trip_mode <- nil;
        current_purpose <- nil;
    }

    action settle_trip_state(building arrival_building, string status) {
        building resolved_arrival <- arrival_building;
        if (resolved_arrival = nil) {
            resolved_arrival <- (the_final_target != nil) ? the_final_target : the_target;
        }
        if (resolved_arrival != nil) {
            location <- point(resolved_arrival);
        }
        if (current_trip_id != nil) {
            do end_trip_log(status);
        }
        the_target <- nil;
    ...
    init {
        ...
        if (working_place != nil and (start_work < starting_date.hour or (start_work = starting_date.hour and delayed_start <= starting_date.minute))) {
            self.location <- working_place.location;
            objective <- "working";
            stopped <- true;
            current_origin_type <- working_place.buildingType;
            last_work_departure_day <- starting_date.day;
        } else if (living_place != nil and living_place.buildingType != nil) {
            current_origin_type <- living_place.buildingType;
        }
    }
    ...
    action abort_walking_trip_no_route(string reason_label) {
        ...
        building intended_destination <- (the_final_target != nil) ? the_final_target : the_target;
        do settle_trip_state(intended_destination, "ABORTED_NO_ROUTE_WALK");
    }
    ...
    reflex time_to_go_home when: objective = "working" 
        ...
        objective <- "resting";
        newObjective <- true;
        mandatory_home_return_pending <- true;
    ...
    reflex do_activities when: objective = "resting"
                                              and !newObjective
                                              and the_target = nil
                                              and current_trip_id = nil
                                              and !startJourney
                                              and !isMoving
                                              and !waitingForTrain
                                              and !mandatory_home_return_pending
                                              and current_date.hour >= end_work
                                              and current_date.hour + 1 < bedtime
                                              and current_date.hour >= hourIsFree
                                              and stopped {
    ...
    action instantiate_car {
        if (objective != "accompany") {
            ...
            create normalCars {
                ...
                initialCrossroad <- (crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation) at_distance (initialSearchRadius #meters) closest_to(myself.location));
                if (initialCrossroad = nil) {
                    initialCrossroad <- (crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation) closest_to(myself.location));
                }
                targetCrossroads <- (crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation) at_distance (initialSearchRadius #meters) closest_to(myself.the_target));
                if (targetCrossroads = nil) {
                    targetCrossroads <- (crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation) closest_to(myself.the_target));
                }
            ...
            loop t over: availableTaxis {
                if (length(pendingTrips) > 0) {
                    ...
                    if (selectedTrip != nil) {
                        ...
                        t.targetCrossroads <- (crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation) at_distance (initialSearchRadius #meters) closest_to selectedTrip.passenger.location);
                        if (t.targetCrossroads = nil) {
                            t.targetCrossroads <- (crossroads where (!each.isStreet and !each.crossroadsNoInitialLocation) closest_to selectedTrip.passenger.location);
                        }
                        t.rideRequest <- true;
                        t.isAvailable <- false;
