...
global {
	...
	string statisticsCity <- "28074 LeganÃ©s";
...
string sample_household_type_by_size(int household_size) {
    string selected <- "";
    if (household_size = 1 and !empty(keys(householdStructureProbabilities1Person))) {
        selected <- rnd_choice(householdStructureProbabilities1Person);
    } else if (household_size = 2 and !empty(keys(householdStructureProbabilities2Persons))) {
        selected <- rnd_choice(householdStructureProbabilities2Persons);
    } else if (household_size = 3 and !empty(keys(householdStructureProbabilities3Persons))) {
        selected <- rnd_choice(householdStructureProbabilities3Persons);
    } else if (household_size = 4 and !empty(keys(householdStructureProbabilities4Persons))) {
        selected <- rnd_choice(householdStructureProbabilities4Persons);
    } else if (household_size >= 5 and !empty(keys(householdStructureProbabilities5Persons))) {
        selected <- rnd_choice(householdStructureProbabilities5Persons);
    }
...
Person create_person_from_spec(string gender_label, string age_range_label, Household household) {
    ...
    int endWork <- startWork + shiftDuration;
    if (endWork < minWorkEnd) { endWork <- minWorkEnd; }
    if (endWork > maxWorkEnd) { endWork <- maxWorkEnd; }
    if (endWork <= startWork) { endWork <- min(23, startWork + 1); }
...
int createFamilies(int s) {
    ...
    map<string, float> age_probs <- map<string, float>(map([]));
    loop ar over: age_ranges {
        float p <- (ageGroupProbabilities[ar] = nil) ? 0.0 : max(0.0, ageGroupProbabilities[ar]);
        age_probs[ar] <- p;
    }
    float age_prob_sum <- 0.0;
    loop ar over: age_ranges { age_prob_sum <- age_prob_sum + age_probs[ar]; }
    if (age_prob_sum <= 0.0) {
        loop ar over: age_ranges { age_probs[ar] <- 1.0; }
    }
    map<string, int> age_counts_target <- largest_remainder_counts(age_probs, age_ranges, numPeople);
    map<string, int> gender_counts_target <- largest_remainder_counts(gender_probs, ["male", "female"], numPeople);

    list<string> age_pool <- [];
    loop ar over: age_ranges {
        loop i from: 1 to: age_counts_target[ar] {
            age_pool <- age_pool + ar;
        }
    }
    list<string> gender_pool <- [];
    loop g over: ["male", "female"] {
        loop i from: 1 to: gender_counts_target[g] {
            gender_pool <- gender_pool + g;
        }
    }
    age_pool <- shuffle(age_pool);
    gender_pool <- shuffle(gender_pool);

    int person_cursor <- 0;
    households <- [];
    loop hh_size over: household_plan {
        int hh_size_i <- int(hh_size);
        string hh_size_label <- household_label_from_size(hh_size_i);
        if (person_cursor >= numPeople) { break; }
        create Household number: 1 returns: household {
            numberPersons <- hh_size_label;
            ...
            if (!livesInLeganes) {
                ...
                if (leganesEntry != nil and !(leganesEntry = "Leganes")) {
                    building externalHouse <- first(building where (each.buildingType = "city" and each.buildingName = leganesEntry));
                    if (externalHouse != nil) { house <- externalHouse; }
                }
            }
        }
        list<Person> householdMembers <- [];
        loop slot from: 1 to: hh_size_i {
            if (person_cursor < numPeople) {
                string ar <- age_pool[person_cursor];
                string g <- gender_pool[person_cursor];
                Person p <- create_person_from_spec(g, ar, household[0]);
                householdMembers <- householdMembers + p;
                person_cursor <- person_cursor + 1;
            }
        }

        list<Person> adults <- householdMembers where (each.age >= 18);
        list<Person> minors <- householdMembers where (each.age < 18);
        if (length(adults) >= 2) {
            adults[0].partner <- adults[1];
            adults[1].partner <- adults[0];
        }
        Person mother_ref <- one_of(adults where (each.gender = "female"));
        Person father_ref <- one_of(adults where (each.gender = "male"));
        if (mother_ref = nil and length(adults) > 0) { mother_ref <- adults[0]; }
        if (father_ref = nil and length(adults) > 1) { father_ref <- adults[1]; }
        loop child over: minors {
            if (mother_ref != nil) {
                child.mother <- mother_ref;
                mother_ref.children <- mother_ref.children + child;
            }
            if (father_ref != nil and father_ref != mother_ref) {
                child.father <- father_ref;
                father_ref.children <- father_ref.children + child;
            }
        }

        household[0].members <- list<Person>(householdMembers);
        household[0].householdType <- sample_household_type_by_size(hh_size_i);
        households <- households + household;
    }
    write("Synthetic households created: " + length(households) + " | persons generated: " + length(Person));
...
// getPerson function: creates a person with his or her characteristics
	...
	Person getPerson(string sex, int minAge, int maxAge, list<Person> children, string householdType, Household household, bool decreasingBias) {
	    ...
	    // Determine canonical gender in English
	    string raw_gender <- (sex = nil) ? rnd_choice(sexProbabilities) : sex;
...
species databaseReader skills: [SQLSKILL] {
    ...
    action calculateHouseholdPercentages(string municipality) {
        list<list> results <- list<list>(select(
            params: PARAMS, 
            select: "WITH TotalHogarMunicipio AS ( SELECT SUM(Total) AS TotalMunicipio FROM Hogares WHERE Municipio = ? AND TamanoHogar NOT IN ('Total', 'Total (tamaÃƒÆ’Ã‚Â±o del hogar)') ) SELECT TamanoHogar, ROUND(SUM(Total) * 1.0 / (SELECT TotalMunicipio FROM TotalHogarMunicipio), 2) AS Porcentaje FROM Hogares WHERE Municipio = ? AND TamanoHogar NOT IN ('Total', 'Total (tamaÃƒÆ’Ã‚Â±o del hogar)') GROUP BY TamanoHogar;", 
    ...
    init {
        ...
        householdStructureProbabilities5Persons <- readHouseholdStructureProbabilities(statisticsCity, "5 o mÃƒÂ¡s personas");
